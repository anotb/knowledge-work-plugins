...
**Key Assumptions**
- List 3-5 assumptions made due to incomplete client context
- Flag sensitivity: "If [assumption] is wrong, [which recommendation] changes"
- Identify data that would resolve the highest-sensitivity assumptions
...
## Worked Example: Multi-Framework Application

**Prompt**: "Should our mid-size regional bank ($5B assets) expand into wealth management?"

**Key Assumptions**: Client has $5B in assets, limited existing WM capability, strong retail deposit base in 3-state footprint. If assets are <$2B, organic build becomes infeasible (sensitivity: high).

**Five Forces on Wealth Management Industry** (abbreviated)

| Force | Rating | Key Factor |
|---|---|---|
| New Entrants | Medium | Robo-advisors lowering barriers; regulatory licensing still meaningful |
| Buyer Power | High | HNW clients have many options; switching costs are low |
| Supplier Power | Medium | Talent market is tight; experienced advisors command 40-45% payout ratios |
| Substitutes | High | Self-directed investing (Schwab, Fidelity), robo-advisors (~$1.5T AUM) |
| Rivalry | High | Fee compression ongoing (~3-5% annually); incumbents have scale |

**So What?** The WM industry is structurally challenging for new entrants. Profit pools are concentrating toward scale players and niche specialists. A mid-size bank must choose: niche (HNW in footprint) or don't enter.

**VRIO Assessment** (abbreviated)

| Resource | V | R | I | O | Implication |
|---|---|---|---|---|---|
| Existing client relationships (~50K retail clients) | Y | N | N | Y | Parity (not rare, but organized to cross-sell) |
| Local market trust / brand | Y | Y | Y | Partial | Temporary advantage (not fully organized to capture in WM) |
| Deposit/lending data on client finances | Y | Y | Y | N | Untapped advantage (data exists but WM capability to use it doesn't) |

**So What?** The bank's strongest asset is proprietary financial data on existing clients, but it lacks the organizational capability ("O") to capture this in WM. Building the "O" is the critical investment.

**Cross-Framework Synthesis**
- **Converging**: Both frameworks agree the bank shouldn't compete head-on with Schwab/Merrill. The opportunity is niche: clients with $500K-$5M who value local relationships and integrated banking+WM.
- **Contradiction**: Five Forces says the industry is unattractive; VRIO says the bank has underexploited assets. Resolution: the industry is unattractive *for generic entrants* but attractive for a bank that can convert existing relationships. The relevant question is conversion rate, not industry averages.
- **Blind spot**: Neither framework captures regulatory burden (compliance infrastructure for RIA/BD) or the cultural shift from banking to advisory.

**Key Risks**

| Risk | Likelihood | Impact | Mitigation |
|---|---|---|---|
| Advisor recruitment failure | High | High | Acqui-hire a small local RIA ($15-40M) rather than recruit individually |
| Cross-sell conversion <5% | Medium | High | Pilot with top 500 clients before full launch; set 90-day gate |
| Fee compression accelerates | High | Medium | Lead with planning fees, not AUM fees; differentiate on integrated banking |

Single point of failure: if the bank cannot recruit/acquire experienced advisors, nothing else matters.

**Recommendation**: Enter via acquisition of a local RIA ($15-40M), targeting $500K-$5M clients in footprint. Phase 1: acquire, retain existing AUM ($200-500M). Phase 2: cross-sell to existing bank clients (target 8-12% conversion, $500M-$1B incremental AUM in 24 months). Break-even at ~$800M AUM. Go/no-go decision hinges on finding an acquisition target with <15% client concentration risk.
